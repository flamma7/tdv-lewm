...
class SaveCkptCallback(Callback):
    ...
    def on_sanity_check_end(self, trainer, pl_module):
        ...
        if trainer.is_global_zero and self.hf_cfg.get('enabled'):
            self._save(pl_module.model, 0)
    ...
    def _get_hf_api(self):
        ...
        token = UserSecretsClient().get_secret(
            self.hf_cfg.get('token_secret', 'HF_TOKEN')
        )
        # token = self.hf_cfg.get('token_secret') # just for running locally
    ...
    def _upload_to_hf(self, weights_file):
        ...
        prefix = self.hf_cfg.get('path_prefix') or ''
        ...
        print(f'Uploaded {weights_file} to {repo_id}')
